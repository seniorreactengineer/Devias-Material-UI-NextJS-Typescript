...
interface FilterItem {
  label: string;
  field: "id" | "name" | "category" | "status" | "inStock";
  value: unknown;
...
export const ProjectListFilters: FC<ProjectListFiltersProps> = (props) => {
  ...
  const [queryNameValue, setQueryNameValue] = useState<string>("");
  const [queryIDValue, setQueryIDValue] = useState<string>("");
  ...
  const handleQueryNameChange = (event: ChangeEvent<HTMLInputElement>): void => {
    setQueryNameValue(event.target.value);
  };

  const handleQueryNameKeyup = (event: KeyboardEvent<HTMLInputElement>): void => {
    if (event.code === "Enter" && queryNameValue) {
  ...
  const handleQueryIDChange = (event: ChangeEvent<HTMLInputElement>): void => {
    setQueryIDValue(event.target.value);
  };

  const handleQueryIDKeyup = (event: KeyboardEvent<HTMLInputElement>): void => {
    if (event.code === "Enter" && queryIDValue) {
      // We only allow one chip for the name field

      const filterItem = filterItems.find(
        (filterItem) => filterItem.field === "id"
      );
